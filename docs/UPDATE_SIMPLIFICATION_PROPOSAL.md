...
Status: **accepted and implemented**, 2026-09-19. It walks back part of
milestone 5; `PILOT_DESIGN_M5_DELIVERY.md` records what was withdrawn.

**One item in the Remove list below was wrong and was not carried out.**
Auto-apply at a launch or foreground boundary is listed for removal, but it is
not part of the coordination layer -- it is the behaviour that makes an
installed app current when it is reopened, which is exactly what matters on a
phone. Removing it broke `lifecycle.spec.ts:180` and `:303`, which is how the
error was caught. What was removed is its _gate_: the `prepareUpdate()` call
that polled every open window and cancelled when any of them had a form open.
Boundaries now apply a waiting release unconditionally.
...
- Reload orchestration: `reloadFor`, `intended`, `intendedWorker` and the
  bounded retry. (Not the auto-apply-at-entry path -- see the correction
  above.)
