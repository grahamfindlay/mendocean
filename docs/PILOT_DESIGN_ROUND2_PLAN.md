...
Two preferences were asked separately. Both are now confirmed by the user:

- Name the third forecast view **Rows**, because it includes both practices and independent rows. Shipped under that name in M7.
- Past shows independent rows and attending practices by default, **plus any row with a saved report regardless of BHC attendance**. Keep an optional Show all practices control for correcting/importing historical records. Otherwise, a later BHC attendance change could hide an existing log. Confirmed September 19, 2026; R30 builds this exception rather than treating it as a question.
...
| R20 | ~~Hide Week's start time and **Duration** controls behind a small labeled Change time disclosure.~~ Superseded: with fixed practice windows on Week and scheduled durations on Rows, no arbitrary time remains to change. Satisfied by removal in M7.                                                                                                                                | M7 / **Superseded** |
...
| R23 | ~~Allow dragging highlight boundaries, with discrete time snapping, minimum duration, midnight/DST handling and distinct touch targets.~~ Dropped September 19, 2026. The highlighted window now belongs to a scheduled row, so dragging its edge means rescheduling that row from a chart — write semantics, conflicts and undo. A separate proposal, not the last increment of M7. | M7 / **Dropped**    |
...
| R30 | Past default includes attending practices and independent rows; retain saved-report visibility under the proposed exception and optional historical override.                                                                                                                                                                                                                        | M8 / Medium         |
...
**R20 is superseded and is not implemented.** It hides Week's start time and
Window behind a Change time disclosure, a control that existed only because the
cards were keyed to a typed instant. Fixed practice windows remove that instant,
so there is nothing in Week for the disclosure to hide. The capability it
presented -- the same clock time compared across days -- is withdrawn
deliberately, not lost: "There does not need to be arbitrary-time forecasting.
It should be possible to schedule independent rows for any time, and then that
row will show up as a card under Forecasts." Step 6 completes that replacement.
...
**R23 was dropped**, September 19, 2026. R22 shipped and was confirmed; the
premise R23 was written under did not survive R24. A dragged boundary used to
change a number in a planner's text field. The highlighted window now belongs to
a scheduled row, so dragging its edge reschedules that row from a chart, which
needs write semantics, conflict handling and undo. That deserves its own
proposal.
