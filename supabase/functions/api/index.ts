...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (path === "outing") {
      ...
      const saved = check(await client.rpc("create_outing", { body: outing }));
    ...
    if (path === "bhc/connect") {
      const parsed = z
        ...
        .parse(input);
      const auth = await bhcGet("authenticate/checkApiKey", parsed.token);
      const custid = Number(auth.custid);
      if (!custid)
        throw new HttpError(400, "BHC did not recognize this token.");
      const clubs = list(await bhcGet("users/getAllWhitelabels", parsed.token));
