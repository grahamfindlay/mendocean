create or replace function public.pilot_health() returns jsonb language sql security definer set search_path='' as $$
select jsonb_build_object(
 ...
 'models',(select coalesce(jsonb_agg(jsonb_build_object('id',id,'created_at',created_at,'status',status,'metrics',metrics,
   'eligible',coalesce(artifact->>'eligible'='true',false),
   'current_revision',coalesce((artifact->>'dataset_revision')::bigint=(select revision from private.dataset_state where singleton),false)) order by created_at desc,id desc),'[]') from private.model_runs)
) $$;
